...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-KCWODOFD.js");
  const signature_ = (() => {
...
async function sendTransaction(client, parameters) {
  ...
  try {
    ...
    if ((account == null ? void 0 : account.type) === "local") {
      ...
      const transactionEnvelope = (_g = (_f = chain ?? client.chain) == null ? void 0 : _f.serializers) == null ? void 0 : _g.transactionEnvelope;
...
async function getWebSocketRpcClient(url, options = {}) {
  ...
  return getSocketRpcClient({
    async getSocket({ onClose, onError, onOpen, onResponse }) {
      const WebSocket = await import("./native-GFZV6UKD.js").then((module) => module.WebSocket);
...
function hmac256(key, value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = hmac(sha256, from2(key), from2(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
...
function validate3(value) {
  return value.length === 66 && validate2(value, { strict: true });
}
...
function encodeBytes(value, { type }) {
  ...
  if (!parametersize) {
    ...
    return {
      dynamic: true,
      encoded: concat3(padLeft(fromNumber2(bytesSize, { size: 32 })), value_)
    };
  }
...
function encodeString(value) {
  ...
  return {
    dynamic: true,
    encoded: concat3(padRight(fromNumber2(size3(hexValue), { size: 32 })), ...parts)
  };
}
...
__export(Secp256k1_exports, {
  InvalidSeedSizeError: () => InvalidSeedSizeError,
...
// node_modules/viem/node_modules/ox/_esm/core/internal/keyDerivation.js
function derive(seed, domain, options = {}) {
  const { validate: validate9 } = options;
  for (let counter = 0; ; counter++) {
    const key = hmac256(seed, concat2(domain, fromNumber(counter, { size: 4 })), { as: "Bytes" });
    if (!validate9 || validate9(key))
      return key;
    key.fill(0);
  }
}

// node_modules/viem/node_modules/ox/_esm/core/internal/mnemonic.js
function toPrivateKey(mnemonic, options = {}) {
  const { path = "m/44'/60'/0'/0/0", passphrase } = options;
  const privateKey = HDKey.fromMasterSeed(toSeed(mnemonic, { passphrase })).derive(path).privateKey;
  if (options.as === "Bytes")
    return privateKey;
  return fromBytes(privateKey);
}
...
function fromMnemonic(mnemonic, options = {}) {
  ...
  return toPrivateKey(mnemonic, { as, passphrase, path });
}
function fromSeed(seed, options = {}) {
  const { as = "Hex" } = options;
  const bytes = from2(seed);
  if (bytes.length < 32)
    throw new InvalidSeedSizeError({ size: bytes.length });
  const privateKey = derive(bytes, fromSeedDomain, {
    validate: noble.utils.isValidPrivateKey
  });
  if (as === "Hex") {
    const value = fromBytes(privateKey);
    privateKey.fill(0);
    return value;
  }
  return privateKey;
...
var InvalidSeedSizeError = class extends BaseError2 {
  constructor(options) {
    ...
    Object.defineProperty(this, "name", {
      ...
      configurable: true,
      writable: true,
      value: "Secp256k1.InvalidSeedSizeError"
    });
  }
};
...
function wrap(value) {
  ...
  const suffixLength = fromNumber2(size3(suffix), { size: 32 });
...
async function simulateCalls(client, parameters) {
  ...
  const getBalanceData = account ? encode4(from12("constructor(bytes, bytes)"), {
    ...
    args: [
      getBalanceCode,
      encodeData(from14("function getBalance(address)"), [account.address])
    ]
  }) : void 0;
...
function wrap2(value) {
  ...
  return concat3(encode3(from6("address, bytes, bytes"), [
    to2,
    data,
    signature
  ]), magicBytes2);
...
//# sourceMappingURL=chunk-NX26IW2G.js.map
